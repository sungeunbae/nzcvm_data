"""
Process large tomography parquet files: load by elevation to avoid memory overflow.

Optimized for large files by:
- Loading row groups (chunks) instead of entire file
- Processing one elevation at a time
- Discarding data after each elevation to free memory
"""
...
def load_parquet_data_chunked(input_file: Path) -> pd.DataFrame:
    """Load entire parquet file using row groups (memory efficient)."""
    ...
    for row_group_idx in range(parquet_file.num_row_groups):
        ...
        chunks.append(chunk)
        print(f"     Loaded row group {row_group_idx + 1}/{parquet_file.num_row_groups} ({len(chunk):,} rows)")
...
def create_grid_from_data(df: pd.DataFrame, spacing_km: float, padding_km: float = 10.0) -> tuple:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Process large tomography parquet file: load by row groups, process elevation by elevation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # 1. First inspect the parquet file structure:
  python inspect_parquet.py DB2025_NZTM.parquet
  
  # 2. Then process with identified columns:
  python %(prog)s DB2025_NZTM.parquet --x-nztm-col 11 --y-nztm-col 12 \\
    --depth-col 8 --vp-col 0 --vs-col 2 --rho-col 3
  
  # 3. With interpolation:
  python %(prog)s DB2025_NZTM.parquet --x-nztm-col 11 --y-nztm-col 12 \\
    --depth-col 8 --vp-col 0 --vs-col 2 --rho-col 3 \\
    --interpolate --spacing 1.4
        """
    )
    ...
    print("=" * 70)
    print("TOMOGRAPHY DATA PROCESSOR (NZTM) - PARQUET VERSION")
    print("=" * 70)
    ...
    try:
        ...
        print(f"   Please verify your column index arguments")
        raise
    ...
    if x_range < 100 or y_range < 100:
        ...
        print(f"   Please verify your column indices!")
